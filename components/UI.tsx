...
export const AudioPlayer = ({ src, label = "Audio" }) => {
  ...
  if (!src) return <div className="flex items-center gap-2 text-gray-400 dark:text-slate-600 text-xs italic"><Volume2 size={16} className="opacity-50" /><span>No audio available</span></div>;
  return (
    <div className="relative flex items-center">
      <button onClick={playAudio} disabled={isPlaying} className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-medium transition-colors ${isPlaying ? 'bg-amber-100 dark:bg-amber-900 text-amber-800 dark:text-amber-100' : 'bg-slate-800 dark:bg-slate-700 text-white hover:bg-slate-700 dark:hover:bg-slate-600 shadow-sm'}`}>
        <Volume2 size={16} className={isPlaying ? 'animate-pulse' : ''} /> {isPlaying ? 'Playing...' : label}
      </button>
...
export const Modal = ({ title, children, onClose }: any) => (
   <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-sm shadow-2xl p-6 animate-scale-in max-h-[90vh] overflow-y-auto border dark:border-slate-800">
